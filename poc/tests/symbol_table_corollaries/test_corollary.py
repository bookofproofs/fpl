...
"""
Tests of FPL corollaries and how they are represented in the symbol table
"""


class CorollariesTests(unittest.TestCase):
    maxDiff = None
    path = None
    ...
    def test_possibilities(self, use_case):
        ...
        if AuxISourceAnalyser.verbose:
            print(actual)
            if CorollariesTests.rewrite and result[1].strip() != actual:
                self.util.rewrite_expected_test_case(self.path_to_usecases, use_case, actual)
        else:
            if CorollariesTests.rewrite:
                self.assertEqual(CorollariesTests.rewrite,
